...
import { BeasiswaData } from "@/app/lib/types";
import ListBookmark from "@/app/lib/listbookmark";

const DetailPage = ( { params } : { params : { id : string } } ) => {
  const [detailData, setDetailedData] = useState<BeasiswaData | null>(null);
  ...
  const handleClick = async () => {
    ...
    if (!isClicked) {
      await fetch("/api/bookmark/add", {
        method: "POST",
        body: JSON.stringify({type: "beasiswa", itemID: params.id})
      })
    } else {
  ...
  useEffect(() => {
    const process = async () => {
      const data = await GetBeasiswa(params.id);
  ...
  return (
    <div className="my-20">
      <Navbar />
      <div className="mx-20">
        <HeaderWithBackButton title="Detail Beasiswa" iconType="detail"/>
      </div>
